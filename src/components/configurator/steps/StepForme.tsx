...
const SHAPES: Array<{ id: ShapeId; label: string; glyph: React.ReactNode }> = [
  ...
  { id: 'carre', label: 'Carré', glyph: <rect x="8" y="8" width="24" height="24" /> },
...
/** Étape 3 — cinq silhouettes dessinées au trait, aucun cadre. */
export default function StepForme() {
  const { config, update } = useConfigurator();

  return (
    <div className="grid grid-cols-3 gap-x-4 gap-y-10 sm:grid-cols-5 sm:gap-x-8">
      {SHAPES.map((shape) => {
